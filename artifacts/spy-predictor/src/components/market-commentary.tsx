...
import { MessageSquare, TrendingUp, TrendingDown, Minus, AlertTriangle, CheckCircle, Clock, Activity } from "lucide-react";
...
export function MarketCommentary({ mtfData, intradaySignal, swingSignal, isLoading }: Props) {
  ...
  const isMixed = align?.direction === "mixed" || align?.direction === "neutral";

  // Entry quality pill color
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {z?.entryQuality && (
            <Pill color={qualityColor}>
              {z.entryQuality} Entry
            </Pill>
          )}
          {z?.riskRating && (
            <Pill color={z.riskRating === "High" ? "border-bearish/40 text-bearish bg-bearish/5" : z.riskRating === "Medium" ? "border-orange-400/40 text-orange-400 bg-orange-400/5" : "border-bullish/40 text-bullish bg-bullish/5"}>
              {z.riskRating} Risk
            </Pill>
          )}
          {isMarketOpen ? (
            <Pill color="border-bullish/40 text-bullish bg-bullish/5">
              <span className="w-1 h-1 rounded-full bg-bullish mr-1 animate-pulse inline-block" />
              Market Open
            </Pill>
          ) : (
      ...
      <div className="p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-3">
          ...
          {/* Time window */}
          {z?.entryWindow && (
            <div className={`rounded-xl p-3 border text-xs ${z.entryWindow.isOptimal ? "bg-bullish/5 border-bullish/20" : z.entryWindow.isDanger ? "bg-bearish/5 border-bearish/20" : z.entryWindow.isCaution ? "bg-orange-400/5 border-orange-400/20" : "bg-muted/5 border-muted/15"}`}>
          ...
          {tf5 && (
            <div className="rounded-xl p-3 border border-white/5 bg-white/[0.02] text-xs space-y-2">
              ...
              <div className="text-muted-foreground/70">
                EMA8 ${tf5.ema8.toFixed(2)} {tf5.ema8 > tf5.ema21 ? "above" : "below"} EMA21 ${tf5.ema21.toFixed(2)} — <span className={tf5.ema8 > tf5.ema21 ? "text-bullish" : "text-bearish"}>{tf5.ema8 > tf5.ema21 ? "bullish crossover" : "bearish crossover"}</span>
              </div>
            </div>
          )}
          ...
          {z?.tradingAdvice && z.tradingAdvice.length > 0 && (
            <div className="rounded-xl p-3 border border-white/5 bg-white/[0.02] text-xs space-y-2">
              ...
              {z.tradingAdvice.map((advice, i) => {
                const isWarning = advice.toLowerCase().includes("caution") || advice.toLowerCase().includes("wait") || advice.toLowerCase().includes("reduce") || advice.toLowerCase().includes("risky");
                const isBullish = advice.toLowerCase().includes("call") && !isWarning;
                return (
