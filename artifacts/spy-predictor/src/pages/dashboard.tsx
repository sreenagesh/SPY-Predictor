    ...
    return (
      <div className="w-full h-[60vh] flex flex-col items-center justify-center">
        <div className="bg-destructive/10 text-destructive border border-destructive/20 p-6 rounded-2xl max-w-md text-center">
          ...
          <p className="text-sm opacity-80">Failed to connect to the predictive model API. Please try again later or check your backend services.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "SPY Current", value: spyData?.currentPrice ? formatCurrency(spyData.currentPrice) : "---", sub: "Live Data" },
          { 
          ...
          { label: "Last Updated", value: prediction?.timestamp ? new Date(prediction.timestamp).toLocaleTimeString() : "---", sub: "Model Sync" },
          ...
          >
            <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-1">{stat.label}</span>
            <span className={`text-2xl font-mono font-bold ${stat.isPos ? 'text-bullish' : stat.isNeg ? 'text-bearish' : 'text-foreground'}`}>
              {stat.value}
            </span>
